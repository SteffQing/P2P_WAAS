...
const HOTWALLET_ADMIN_KEY = getEnvVariable("HOTWALLET_ADMIN") as `0x${string}`;
...
function deploymentArgs() {
  ...
  const HOTWALLET_ADMIN = privateKeyToAddress(HOTWALLET_ADMIN_KEY);
  return [SMARTTWALLET_ADMIN, HOTWALLET_ADMIN, DEPLOYER_ADMIN] as const;
}
